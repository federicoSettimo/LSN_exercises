...
int main(int argc, char** argv){
  bool odlconfig;
  string errstr = "\nTo run with only actual spatial configuration (from config.0)\n\t./MolDyn_NVE 0\nTo run with also old spatial configuration (from old.0)\n\t./MolDyn_NVE 1\n\n";
  if (argc == 1) {
    cout << "Running using only the actual spatial configuration" << endl;
    cout << "To rerun with also old spatial configuration run \"./MolDyn_NVE 1\"" << endl << endl;
    odlconfig = false;
  }
  else if (argc > 2) {
  ...
  if (argc > 1) {
    string param(argv[1]);
    if (param == "0") {
      cout << "Running using only the actual spatial configuration" << endl << endl;
      odlconfig = false;
    }
    else if (param == "1") {
      cout << "Running using old and actual spatial configuration" << endl << endl;
      odlconfig = true;
    }
    else{
  ...
  Input(odlconfig);             //Inizialization
  int nconf = 1;
...
void Input(bool oldfconfig){
  ...
  if(oldfconfig == true) {
   ...
   // Writes the number of steps used for writing U,K,E,t
   ofstream out;

   out.open("output_epot.dat");
   out << nstep/10 << endl;
   out.close();
   out.open("output_ekin.dat");
   out << nstep/10 << endl;
   out.close();
   out.open("output_temp.dat");
   out << nstep/10 << endl;
   out.close();
